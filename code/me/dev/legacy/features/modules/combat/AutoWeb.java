...
extends Module {
    ...
    private boolean check() {
        ...
        if (obbySlot == -1) {
            Command.sendMessage("<" + this.getDisplayName() + "> " + (Object)ChatFormatting.RED + "No Webs in hotbar disabling...");
            this.toggle();
            return true;
        }
    ...
    private EntityPlayer getTarget(double range) {
        ...
        for (EntityPlayer player : AutoWeb.mc.world.playerEntities) {
            if (EntityUtil.isntValid((Entity)player, range) || player.isInWeb || Client.speedManager.getPlayerSpeed(player) > 30.0) continue;
            if (target == null) {
                target = player;
