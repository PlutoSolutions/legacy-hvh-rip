...
public class HoleFillUtil {
    ...
    public static void placeBlockScaffold(BlockPos pos) {
        ...
        for (EnumFacing side : EnumFacing.values()) {
            ...
            if (!HoleFillUtil.canBeClicked(neighbor) || !(eyesPos.squareDistanceTo(hitVec = new Vec3d((Vec3i)neighbor).add(0.5, 0.5, 0.5).add(new Vec3d(side2.getDirectionVec()).scale(0.5))) <= 18.0625)) continue;
    ...
    private static float[] getLegitRotations(Vec3d vec) {
        ...
        return new float[]{HoleFillUtil.mc.player.rotationYaw + MathHelper.wrapDegrees((float)(yaw - HoleFillUtil.mc.player.rotationYaw)), HoleFillUtil.mc.player.rotationPitch + MathHelper.wrapDegrees((float)(pitch - HoleFillUtil.mc.player.rotationPitch))};
    }
