...
public class TestUtil {
    ...
    public static boolean placeBlock(BlockPos pos) {
        if (TestUtil.isBlockEmpty(pos)) {
            ...
            for (EnumFacing f : facings = EnumFacing.values()) {
                ...
                if (emptyBlocks.contains((Object)neighborBlock) || !(TestUtil.mc.player.getPositionEyes(mc.getRenderPartialTicks()).distanceTo(vec) <= 4.25)) continue;
    ...
    public static boolean isBlockEmpty(BlockPos pos) {
        try {
            if (emptyBlocks.contains((Object)TestUtil.mc.world.getBlockState(pos).getBlock())) {
                Entity e;
    ...
    public static boolean canPlaceBlock(BlockPos pos) {
        if (TestUtil.isBlockEmpty(pos)) {
            EnumFacing[] facings;
            for (EnumFacing f : facings = EnumFacing.values()) {
                if (emptyBlocks.contains((Object)TestUtil.mc.world.getBlockState(pos.offset(f)).getBlock())) continue;
